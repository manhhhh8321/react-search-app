...
describe("SearchResult Component", () => {
  ...
  test("displays correct number range in results", () => {
    ...
    expect(screen.getByText("Showing 11 - 12 of 50 results")).toBeInTheDocument();
  });

  test("renders no results message when items are empty", () => {
    render(
      <SearchResult items={[]} total={0} page={1} pageSize={5} searchKeyword="test" />
    );
  ...
  test("renders highlighted text in title and excerpt", () => {
    ...
    expect(extractHighlightFromDocument).toHaveBeenCalledWith("Test Title 1", []);
    expect(extractHighlightFromDocument).toHaveBeenCalledWith("Excerpt 1", []);
  });
});
